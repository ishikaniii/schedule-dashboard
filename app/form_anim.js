...
(function (global) {
  ...
  const EX = {
    ...
    // latZ：閉じた状態(胸の上）は狭く、開いた状態は左右に大きく広がる。
    "ダンベルフライ": {
      mode: "root", root: [0, 0.5], cam: [-0.1, 0.75, 3.8],
      A: A(-90, 100, 0, 120, 178), B: A(-90, 100, 0, 70, 178), latZ: [0.16, 0.62],
    ...
    "シュラッグ": {
      ...
      A: A(0, 0, 0, 5, 5), B: A(0, 0, 0, 5, 5), gz: 0.2, shrug: [0, 0.09],
      labels: ["肩をすくめる（耳に近づける）", "ゆっくり戻す"],
      equip: [{ t: "bar", at: "wr", r: 0.18 }],
    },

    // -- 肩 --
    // 実際は体の正面方向（横）への動きなので、腕の前後角度はほぼ変えず、latZ（奥行き＝左右の
    // 広がり）で「横に持ち上げる」動きを表す（2026-09-22、前後の動きになっていた指摘で修正）。
    "サイドレイズ": {
      mode: "ankle", cam: [0, 1.0, 3.6],
      A: A(0, 0, 0, 5, 5), B: A(0, 0, 0, 20, 10), latZ: [0.16, 0.62],
    ...
    "リアレイズ": {
      ...
      A: A(60, 20, -5, 15, 15), B: A(60, 20, -5, 25, 20), latZ: [0.16, 0.6],
      labels: ["体を前に倒し、腕を横へ開く", "ゆっくり下ろす"],
      equip: [{ t: "db", at: "wr" }],
    },
    "アップライトロウ": {
      mode: "ankle", cam: [0, 1.0, 3.6],
  ...
  function update(ex, k) {
    const { q, j } = pose(ex, k);
    // shrug（2026-09-22追加）：シュラッグ用。このrigには「肩をすくめる」専用の関節が無い
    // （腰・膝・肘の角度だけでは表現できない）ため、肩から先（肩・肘・手首）だけを、
    // 股関節・脚は動かさずに、少しだけ持ち上げて表す。
    if (ex.shrug) {
      const sr = ex.shrug[0] + (ex.shrug[1] - ex.shrug[0]) * k;
      j.sh = [j.sh[0], j.sh[1] + sr]; j.el = [j.el[0], j.el[1] + sr]; j.wr = [j.wr[0], j.wr[1] + sr];
    }
    const zh = S.zHip, zs = S.zSh;
    // latZ（2026-09-22追加）：サイドレイズ・ダンベルフライ等、実際は体の正面方向（左右）に
    // 動く種目のための、手の奥行き(z)をA→B間でアニメーションさせる値。これが無い種目は、
    // 従来どおり ex.gz（一定の握り幅）を使う——このrigは元々、前後・上下の動き（同じ平面内）
    // しか作れなかったため、左右の動きはこれまで表現できていなかった（見た目が横に開かない、
